# Deploying Wakegraph for end users to test

Wakegraph ships as **one self-contained app**: the server also serves the dashboard, so a tester
...
Produces a portable folder with `Wakegraph.exe` and a bundled Java runtime — **the tester needs
...
Zip the `server/build/desktop/Wakegraph` folder and send it. **The tester:** unzip → run
`Wakegraph.exe` → the browser opens on the dashboard → the first-run wizard walks them through
...
- **Desktop / jar:** if you don't set a key, Wakegraph generates one and keeps it at
